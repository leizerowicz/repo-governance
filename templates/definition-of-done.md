...
- [ ] [PROPOSED from source repo] If the ADR defines its own inline acceptance criteria (beyond the lint requirement), and a PR satisfies those criteria: flip Status to **Accepted** in that same PR
- [ ] [PROPOSED from source repo] If the ADR's Decision section names implementation prerequisites or sequencing constraints: each prerequisite cites a tracking issue number before the PR merges — prerequisites without tracking issues are invisible to the backlog

> **Why this rule exists:** [PROPOSED — replace with your incident. Example pattern from source repo: "An ADR promised a lint and moved to Accepted without it. The violation it was meant to prevent shipped anyway and caused a production outage; a recheck audit found the same violation still live. Separately, an ADR documented four implementation prerequisites with no tracking issues — the work was invisible to the backlog until a human happened to re-read the ADR."]
...
- [ ] `grep -r '<deleted-interface-or-type>' <project docs and source directories>` run and all stale references updated before merging (was: Stale references to deleted types, interfaces, or commands found with `grep` and updated before merging)

> **Why this rule exists:** [PROPOSED — replace with your incident. Example pattern from source repo: "A PR completed an ADR's retirement work but didn't touch the ADR file. The ADR's own Consequences block kept describing the retirement as pending, creating a P1 finding in the very next audit. The audit found what the DoD should have caught."]
...
<!-- [PROPOSED from source repo: more specific — test the entry point, not a sub-component] -->
...
<!-- [PROPOSED from source repo: lint must be wired into CI, not just exist] -->
...
- [ ] [PROPOSED from source repo] If a required field is added to a shared interface or contract: every doc and onboarding guide that shows example objects of that interface is updated in the same PR
- [ ] If this closes a tracked issue: `Fixes #N` is in the PR description — GitHub closes the issue automatically on merge

> **Why this rule exists:** [PROPOSED — replace with your incident. Example pattern from source repo: "A new dispatch path shipped with no integration test. A database constraint and a type cast both passed CI — the unit tests mocked the layer where both lived — and broke in production."]
...
- [ ] Fix is in place
<!-- [PROPOSED from source repo: test the function that failed, not a delegate] -->
...
- [ ] [PROPOSED from source repo] If the migration adds an enforcement-bearing schema element (an access policy, a rate-limit or lifecycle column, a tenancy flag): the consuming code ships in the same PR, **or** the element is registered in a dormant-schema register with a tracking issue and an activation condition. Schema that promises a control nobody built is a defect, not a head start.

> **Why this rule exists:** [PROPOSED — replace with your incident. Example pattern from source repo: "A data-manipulation migration passed CI against an empty database but failed in production where rows had unexpected values. Separately, a schema review found four enforcement mechanisms the schema promised and no code delivered — inert access policies, phantom rate limits, a dead lifecycle column — all silent for months because no control ever asked 'does anything consume this?'"]

---

<!-- [PROPOSED from source repo] New section — review and adapt before committing. Abstracted from two source-repo work types (agent/prompt/seed changes and scheduler state-machine changes); applies to any repo with cron jobs, scheduled CI, or config-driven automation. Delete if your repo has none. -->
...
> **Why this rule exists:** [PROPOSED — replace with your incident. Example pattern from source repo: "A commit fixed the section an audit flagged but left a contradictory example three paragraphs up in the same file — same file, same problem, different lines. Separately, files touched by PRs between audits accounted for all of the next audit's new doc findings: the author didn't sweep for internal contradictions."]

---

### Issue / Epic

<!-- [PROPOSED from source repo] Structure-at-creation rules — an issue is a unit of work only if it's born actionable. See templates/issue-authoring.md for the full schema. -->
...
<!-- [PROPOSED from source repo: mechanism-agnostic wording, docs/audits/ path convention, and cadence proven at daily-weekdays — adjust to your team's capacity] -->
...
<!-- [PROPOSED from source repo] The audit doc is the required artifact -->
...
<!-- [PROPOSED from source repo] Dead-man probe — the audit machinery itself fails silently; see templates/workflows/audit-deadman.yml -->
...
<!-- [PROPOSED from source repo] P2 aging rule -->
...
<!-- [PROPOSED from source repo] Close-out as a required gate -->
...
<!-- [PROPOSED from source repo] Governance health metrics — see templates/governance-health.md. Uncomment the gate below once you have 6+ audit cycles of data and the metrics feel calibrated. -->
