# Two-Phase Audit Lifecycle — enrichment-pipeline
...
**a) After the first paragraph** (the one describing the `audit-enrichment-pipeline` state machine, domains, and PR), insert the two-phase lifecycle description:
...
**c) In the Severity paragraph** (the one starting "**Severity:**"), remove the disposition guidance at the end — it's now covered by the two-phase lifecycle section above. Change:

> P0 = actively misleading or dangerous now (fixed that week); P1 = hours wasted next sprint (next audit cycle); P2 = cosmetic (tracked, reviewed at the next audit). A P2 finding carried across three consecutive audits without a fix or explicit deferral is either filed as a GitHub issue or closed WONT-FIX with written rationale.

To:

> P0 = actively misleading or dangerous now; P1 = hours wasted next sprint; P2 = cosmetic. Dispositions are applied in the remediation session — see the two-phase lifecycle above. A P2 finding carried across three consecutive audits without a fix or explicit deferral is either filed as a GitHub issue or closed WONT-FIX with written rationale.
