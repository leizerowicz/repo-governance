# Competitive Analysis Skill — all governed repos

**Client:** Hopskip (internal)
**Source:** greg/repo-governance session 2026-07-07
**Scope:** Install the competitive-analysis skill — applies to ai-fleet, analytics-infrastructure, enrichment-pipeline, and any future governed repo.

## Context

The original ai-fleet `/competitive-analysis` command was a Claude Code slash command hardcoded with ai-fleet's specific ADRs, source paths, and primitives. It couldn't transfer to other repos without rewriting.

This is the replacement: a **self-discovering skill** that reads the target repo's actual architecture rather than relying on pre-baked knowledge. It discovers the ADR directory, reads all ADRs and architecture docs, discovers the source structure, and synthesizes primitives from what it finds. No repo-specific customization needed — drop it in and it works.

The skill output lands in `docs/watch-items/` (the generic watch-list convention already established). The watch-list sweep in the audit machine picks up watch items from these docs automatically.

## What to do

### 1. Create the skill file

Copy the skill below into `.claude/skills/competitive-analysis/SKILL.md`:
...
4. **Pull open issues:**
   ```bash
   gh issue list --state open --limit 200 --json number,title,labels | jq -r '.[] | "#\(.number) \(.title)"'
   ```
...
```bash
git add .claude/skills/competitive-analysis/SKILL.md
git commit -m "feat: add competitive-analysis skill

Self-discovering skill that reads the repo's ADRs, architecture docs, and
source structure to evaluate an external product. Two parallel agents
(product research + repo context), synthesis, and a concrete decision with
watch-list items for the audit sweep."
```

## Verifiable outcomes

- `test -f .claude/skills/competitive-analysis/SKILL.md` — skill file exists
- `grep -q 'self-discovering' .claude/skills/competitive-analysis/SKILL.md` — self-discovery approach present
- `grep -q 'DISCOVER the architecture' .claude/skills/competitive-analysis/SKILL.md` — Agent B instructs discovery
- `grep -q 'docs/watch-items' .claude/skills/competitive-analysis/SKILL.md` — output lands in watch-items
- No repo-specific ADR names, primitive names, or file paths hardcoded in the skill

## Notes

This prompt is identical for all governed repos — analytics-infrastructure, enrichment-pipeline, and ai-fleet. The skill discovers each repo's architecture at runtime. No per-repo customization needed.

If a repo doesn't have `docs/watch-items/` yet, the skill creates it on first use (Step 3). The watch-items sweep (already installed in all three repos) will pick up watch-list items from analysis docs automatically.
